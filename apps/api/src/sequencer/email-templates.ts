...
export function buildEmail1(lead: Pick<Lead, "companyName" | "industry" | "contactName">) {
  ...
  return {
    // Curiosity over pitch: naming the company and a concrete "why now" gets
    // opened far more often than a generic subject a spam filter has seen
    // a thousand times.
    subject: `A question about how ${lead.companyName} handles follow-up`,
    bodyHtml: `
      <p>${greeting}</p>
      <p>I've been looking at how ${lead.industry ?? "companies like yours"} are using AI for the
      day-to-day work that quietly eats up a team's time. Lead follow-up, reporting, scheduling,
      that kind of thing.</p>
      <p>Wanted to introduce ${"{{org.name}}"}. We help businesses put that kind of automation in place
      without a big platform migration. No pitch here, just flagging it in case it's useful context
      as you're thinking about where AI fits for ${lead.companyName}.</p>
...
export function buildEmail2(lead: Pick<Lead, "companyName" | "industry">, caseStudySummary?: string) {
  return {
    subject: `What worked for another ${lead.industry ?? "team"} we talked to`,
    bodyHtml: `
